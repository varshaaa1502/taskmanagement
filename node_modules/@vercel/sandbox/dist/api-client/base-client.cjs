...
*/
const ErrorResponseBody = zod.z.object({ error: zod.z.object({ message: zod.z.string() }) });
/**
...
async function parse(validator, response) {
	...
	if (!response.ok) {
		...
		return new require_api_error.APIError(response, {
			message: serverMessage ? `Status code ${response.status} is not ok: ${serverMessage}` : `Status code ${response.status} is not ok`,
			json,
			text,
			sessionId,
			sandboxName
